...
class SequenceAlignment:
    ...
    # two-line dp array generation
    def dpGenerate(self, str1, str2, reverse = False):
        ...
        for i in range(1,len(str1)+1):
            dp[i%2][0] = self.delta * i
            for j in range(1,len(str2)+1):
                dp[i%2][j] = min(dp[i%2][j-1] + self.delta,
                                dp[(i-1)%2][j] + self.delta,
                                dp[(i-1)%2][j-1] + self.alpha[self.h[str1[i-1]]][self.h[str2[j-1]]])
            #print(dp[i%2])
        return dp[len(str1)%2] if reverse == False else dp[len(str1)%2][::-1]
    
    def partition(self, str1, str2):
        
        left_str1 = str1[:len(str1)//2]
        right_str1 = str1[len(str1)//2:]
        # do dp for left part
        dp_left = self.dpGenerate(left_str1, str2, reverse = False)
        # do dp for right part
        dp_right = self.dpGenerate(right_str1, str2, reverse = True)
        # return minimum
        dp = [dp_left[i] + dp_right[i] for i in range(len(dp_left))]
        min_index = min(dp)
        return dp.index(min_index)
    
    def efficientProcessing(self,str1,str2):
        if len(str1) <= 2 or len(str2) <=2:
            return self.basicProcessing(str1,str2)
        # find the best point to cut str2
        pivot = self.partition(str1,str2)
        
        # recurrence for left part
        left_str1, left_str2 = self.efficientProcessing(str1[:len(str1)//2],str2[:pivot])
        # recurrence for right part
        right_str1, right_str2 = self.efficientProcessing(str1[len(str1)//2:],str2[pivot:])
